...
L'obbiettivo di questo esperimento è quello di simulare un intero processo di taratura dalla scelta del metodo di test all'analisi del modello per la regressione per poi su tutto questo fare un report che segue tutti i passaggi e tutte le scelte fatte.
...
L'esperimento consiste nel simulare un esperimento di 
...
Il `test di Shapiro` andrebbe sempre seguito da un test delle anomale, come `test di choveniet` o `grafico Box-Plot`, per poter assicurare l'accuratezza del test.

```{r}
# test di chovenie o qualsiasi verifica delle anomalie
...
Un buon metodo per capire se un modello sia adeguato al data set che vogliamo regredire è l'analisi dei residui, essi ci permettono di capire, nel caso di un modello lineare, se stiamo facendo underfitting o overfitting.
...
**Vedere il pattern nel grafico dei residui**
